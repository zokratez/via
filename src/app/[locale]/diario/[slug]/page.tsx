/**
 * Journal article detail — ES locale.
 * Forwards EN traffic to /en/journal/[slug] canonical URL.
 */
...
export async function generateStaticParams({
  ...
  const articles = await loadArticles("es");
...
export async function generateMetadata({
  ...
  if (locale !== "es") {
    return { alternates: { canonical: `${SITE_URL}/en/journal/${slug}` } };
  }
  const article = await loadArticle("es", slug);
  if (!article) return { title: "PACO Peptide" };
  return buildMetadata({
    locale: "es",
    title: `${article.title} — PACO Peptide`,
    description: article.summary,
    pathname: `/es/diario/${slug}`,
    type: "article",
    publishedTime: article.date,
  });
}

export default async function DiarioArticlePage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  ...
  if (locale === "en") {
    redirect({ href: `/journal/${slug}`, locale: "en" });
  }
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/diario" style={backLinkStyle}>
          {t("diario_back")}
        </Link>
